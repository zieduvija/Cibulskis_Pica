		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					UIManager.put("JTextField.arc", 2);
					...
					frame.setIconImage(
							Toolkit.getDefaultToolkit().getImage(Run.class.getResource("/bildes/ikona.png")));
			...
			for (int i = 0; i < duplikatSaraksts.size(); i++) {
				if (!sloti[i % 3].isVisible()) {
					...
					if (duplikatSaraksts.get(i).getGataviba() < 1) {
						...
						String apraksts = duplikatSaraksts.get(i).getIzmers() + ", "
								+ duplikatSaraksts.get(i).getMiklasVeids() + ", " + duplikatSaraksts.get(i).getMerce()
								+ ", " + duplikatSaraksts.get(i).getSiers() + ", " + duplikatSaraksts.get(i).getGala()
								+ ", " + piedevas;

						slotApraksts[i % 3].setText(apraksts);
						slotProcenti[i % 3].setText(duplikatSaraksts.get(i).getGataviba() + " %");
						slotBilde[i % 3].setIcon(new ImageIcon(((duplikatSaraksts.get(i).getBilde().getImage()
								.getScaledInstance(172, 170, java.awt.Image.SCALE_SMOOTH)))));
